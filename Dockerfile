...
#install things needed to install tailscale
...
RUN apt-get update -y && apt-get install -y libstdc++6 openssl libncurses5 locales tailscale
...
COPY --from=builder --chown=nobody:root /app/_build/${MIX_ENV}/rel/tide ./
